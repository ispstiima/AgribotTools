...
def copy_files_monitored(source_path: Path, dest_path: Path, dirs_exist_ok: bool = True) -> bool:
    log.info(f"Copying folder <{source_path.name}> to <{dest_path.name}>")
    ...
    for source_file_path in tqdm(source_path.iterdir(), total=num_files, ascii="░▒█", desc="Copying files"):
